...
class SelectPlaylist extends Component {
  ...
  componentWillMount() {
    ...
    this.props.getAllPlaylistName().then(data => {
      if (data.success) {
  ...
  handleChange(data) {
    this.props.getOnePlaylist(data.value).then(rps => {
      if (rps.success) {
    ...
    return (
      <section className="select-playlist">
        {activePlaylist && (
          <Link
            to={pathUrl}
            title="Go to active playlist"
            className="select-playlist-label"
          >
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    getAllPlaylistName: () => dispatch(get('ownedPlaylist')),
    getOnePlaylist: plTitle => dispatch(get('playlist/' + plTitle))
  };
};
