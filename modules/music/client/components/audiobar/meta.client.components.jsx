...
class MetaInfoPlaylist extends Component {
  shouldComponentUpdate(nextProps) {
    ...
    return (
      pl !== this.props.pl ||
      onPlayIndex !== this.props.onPlayIndex.path ||
      onPlay !== this.props.onPlay.path
    );
  }
  ...
  render() {
    ...
    const cover = onPlay.albumKey
      ? ps.changeSeparator(onPlay.albumKey, '___', '/')
      : null;
    ...
    // Pl
      ...
      modeLabel = 'track';
      path = onPlay.albumKey
        ? '/album/' + onPlay.albumKey
        : '/music/' + ps.removeLast(onPlay.path);
    }
    ...
    return (
      <Link to={path} className={`audioBar-meta-wrap ${this.props.className}`}>
        <div className="audioBar-meta">
          <i
            aria-hidden="true"
            className="icon icon-eye large-hidden medium-hidden"
          />
          <div className="audioBar-img">
            <Img
              title={`${onPlay.publicName} cover`}
