    ...
    .then(function(metadata) {
      try {
        ...
        if (metadata.format.duration) {
          cleanMeta.time = new Date(metadata.format.duration * 1000)
            .toISOString()
            .substr(11, 8);
          if (cleanMeta.time.substr(0, 2) === '00')
            cleanMeta.time = cleanMeta.time.substr(3, 6);
        } else {
          cleanMeta.time = '';
        }
        ...
        cleanMeta.genre = metadata.common.genre
          ? metadata.common.genre.join(', ').split(/\s*[,;\/]\s*/)
          : [];
        ...
        cleanMeta.composer = metadata.common.composer
        ...
        if (metadata.common.track) {
          if (metadata.common.track.no === null) {
            ...
            cleanMeta.trackno = (metadata.common.track.no + '').replace(
              /^0+(?=\d)/,
              ''
            );
          }
          if (metadata.common.track.of === null) {
            ...
            cleanMeta.trackof = (metadata.common.track.of + '').replace(
              /^0+(?=\d)/,
              ''
            );
          }
        ...
        if (metadata.common.disk) {
          if (metadata.common.disk.no === null) {
            ...
            cleanMeta.diskno = (metadata.common.disk.no + '').replace(
              /^0+(?=\d)/,
              ''
            );
          }
          if (metadata.common.disk.of === null) {
            ...
            cleanMeta.diskof = (metadata.common.disk.of + '').replace(
              /^0+(?=\d)/,
              ''
            );
          }
