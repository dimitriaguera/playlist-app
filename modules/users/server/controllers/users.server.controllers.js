...
// AddExt
const Playlist = require(path.resolve(
  './modules/music/server/models/music.server.models'
));
const musicCtrl = require(path.resolve(
  './modules/music/server/controllers/music.server.controllers'
));
// End AddExt
...
exports.login = function(req, res, next) {
  ...
  Promise.coroutine(function*() {
    ...
    if (isValidPwd) {
      ...
      const token = jwt.sign(secureUser, config.security.jwtSecret, {
      ...
      if (err) {
        if (err.name === 'MongoError' && err.code === 11000) {
          ...
          return errorHandler.errorMessageHandler(
            ...
            res,
            next,
            `${username} already exist. Please choose an other account name.`
          );
