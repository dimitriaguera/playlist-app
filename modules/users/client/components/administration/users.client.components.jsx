...
class Users extends Component {
  ...
  deleteUser(name, i) {
    const _self = this;
    this.props.deleteUser(name).then(data => {
      if (data.success) {
    ...
    return (
      <section className="pal">
        ...
        <div className="wrapper-content">
          ...
          <ul className="unstyled">{userList}</ul>
...
const mapDispatchToProps = dispatch => {
  return {
    fetchUsers: () => dispatch(get('users')),
    deleteUser: name => dispatch(del('users/' + name))
  };
};
...
class UserListItem extends Component {
  shouldComponentUpdate(nextProps) {
    return (
      ...
      nextProps.users !== this.props.users || nextProps.index !== this.props.index
    );
  }
      ...
      if (!(hasRole(user, [ADMIN_ROLE]) && currentUser.username === user.username)) {
        return (
          <button
            ...
            onClick={e => handleOpen(e, user.username, index)}
          >
      ...
      <li>
        <Link to={`/user/${user.username}`}>
          ...
          <div className="users-actions">
            <button
              className="btn btn-icon"
              aria-label={`Change user ${user.username} setting's`}
              onClick={e => handleSettings(e, user.username)}
            >
          ...
          <div className="users-roles">{roles}</div>

          <div className="users-updated">
            {user.updated
              ? `Last update on ${dateFormat(
