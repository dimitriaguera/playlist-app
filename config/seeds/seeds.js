...
const User = require(path.resolve(
  './modules/users/server/models/users.server.models.js'
));
...
const checkIfThereAtLeastOneAdminAndCreateIt = function() {
  User.findOne({ roles: { $in: ['ADMIN_ROLE'] } })
    .then(foundOne => {
      ...
      if (!foundOne) {
        return User.create({
          ...
          .then(() => {
            console.log(
              chalk.red(
                `We have created one admin user : ${config.security.defaultAdminId}/${config.security.defaultAdminPassword}`
              )
            );
        ...
        return User.findOne({ username: config.security.defaultAdminId })
          .exec()
          .then(userAdmin => {
            return userAdmin
              .comparePassword('adminpwd')
              .then(test => {
                if (test)
                  console.error(chalk.bgRed('YOU MUST CHANGE THE ADMIN PASS'));
              })
          ...
          try {
            model.create(data, err => {
              if (err) {
                console.error(
                  chalk.red(
                    `Error seeds on creating document ${count} of "${item.model}" model.`
                  )
                );
                console.error(chalk.red(`Error seeds: ${err.message}`));
              } else {
                console.log(
                  `Successfully seed your DB on created document ${count} of "${item.model}" model.`
                );
              }
            });
            ...
            console.error(
              chalk.red(
                `Error seeds on creating document of "${item.model}" model. Model Doesn't exist.`
              )
            );
          }
        },
